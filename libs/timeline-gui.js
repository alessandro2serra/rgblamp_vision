...
Timeline.prototype.drawTrack = function(track, y) {
  ...
  if (track.type == "property") {
	...
	var color;
	if(track.name=="red") {
	    color=rgbToHex(key.value,0,0);
	} else if (track.name=="green") {
	    color=rgbToHex(0,key.value,0);
	} else if (track.name=="blue") {
	    color=rgbToHex(0,0,key.value);
	} else {
	    color=rgbToHex(key.value,key.value,key.value);
	}
	this.drawRombus(this.timeToX(key.time), y - this.trackLabelHeight*0.5,
			this.trackLabelHeight*0.5, this.trackLabelHeight*0.5,
			color , true, true, selected ? "#FF0000" : "#666666");
...
Timeline.prototype.buildInputDialog = function() {
  ...
  //}
    ...
    controls += '<label style="margin-right:10px">Value: ';
    controls += '<span id="valBox">0</span>';
    controls += '<input type="range" min=0 max=255 id="keyEditDialogValue" onchange=\'document.getElementById("valBox").innerHTML=this.value;\' oninput=\'document.getElementById("valBox").innerHTML=this.value;\' onload=\'document.getElementById("valBox").innerHTML=this.value;\'/></label>';
  ...
  this.keyEditDialogValueVal = document.getElementById("valBox");
...
Timeline.prototype.applyKeyEditDialog = function() {
  ...
  if (isNaN(value)) {
    ...
    var selectedOption = 0;//this.keyEditDialogEasing.options[this.keyEditDialogEasing.selectedIndex];
    var easing = 0; //Timeline.easingMap[selectedOption.value] ;
  for(var i=0; i<this.selectedKeys.length; i++) {
...
Timeline.prototype.showKeyEditDialog = function(mouseX, mouseY) {
    ...
    this.keyEditDialogValueVal.innerHTML = this.selectedKeys[0].value;
